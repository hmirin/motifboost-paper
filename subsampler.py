...
def subsampler(
    ...
    if test_str:
        ...
        train_positive_rate = len(train_df[train_df["status"] == True]) / len(train_df)
        ...
        train_positive_df = train_df[train_df["status"] == True]
        ...
        assert sum(train_df["status"] == True) == int(positive_rate * train_size)
        ...
        assert sum(test_df["status"] == True) == int(positive_rate * test_size)
...
# python -m scripts.emerson_subsampler  --to_dir ./data/interim/sampled_repertoires/Emerson/XX/ --times 50 --train_size XX --test_str Keck
